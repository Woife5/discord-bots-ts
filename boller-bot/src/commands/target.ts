...
import { type ChatInputCommandInteraction, EmbedBuilder, SlashCommandBuilder, User } from 'discord.js';
...
function runCommand(user: User | null) {
    if (!user) {
        ...
        return embed().setDescription('Target has been reset. The first user per server will be joined after and bollered! :D After the last user leaves, the bot will also leave the channel.');
    }

    bollerTarget.id = user.id;
    bollerTarget.name = user.username;
    return embed().setDescription(`Set the target user to ${user.username}. Will join each voice channel the user switches to and only leave after all users leave.`);
}
